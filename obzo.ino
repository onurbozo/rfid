//*************DESIGNED BY ONUR BOZO (2018) FOR BUYUK KULTUR SITESI*************//
...
void loop()
{
  ...
  else
  {
    ...
    {
      programMode = true;
      Serial.println(F("Merhaba Sayin ONUR BOZO, Programa giris yapiyorum."));
    ...
    else
    {
      if (findID(readCard))
      {
        Serial.println(F("Hosgeldin KULTUR SITESI SAKINI, gecis izni verildi."));
        granted(1000);     //500 milisaniyede kilitli kapıyı aç.
      }
      else
      {
...
void granted(int setDelay)
{
  ...
  digitalWrite(buzzer, HIGH);
  delay(80);
  digitalWrite(buzzer, LOW);//yeşil led yanık
...
void successWrite()
{
  ...
  digitalWrite(greenLed, LED_ON);
  digitalWrite(buzzer, HIGH);// yeşil ledin yanık olduğundan emin olun.
  delay(200);
  digitalWrite(greenLed, LED_OFF);
  digitalWrite(buzzer, LOW);// yeşil ledin sönük olduğundan emin olun.
  delay(200);
  digitalWrite(greenLed, LED_ON);
  digitalWrite(buzzer, HIGH);// yeşil ledin yanık olduğundan emin olun.
  delay(200);
